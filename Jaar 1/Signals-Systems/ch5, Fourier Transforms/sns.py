...
from sympy import DiracDelta as symdirac
from sympy import Heaviside as symstepfunction
from sympy import fourier_transform
from sympy import inverse_fourier_transform
from sympy import pi as pi_
...
def stepfunction(tvals, zeroval=0):
    '''
    This is my attempt at making a stepfunction/heaviside.
    ...
    # at the moment, I do not need the increased performance. with small (~10k elements) arrays it is in the ms range difference
    # TODO: when situation occurs where this is too slow, profile and optimize
    if isinstance(tvals, int) or isinstance(tvals, float):
        if tvals < 0:
            return 0
        elif tvals == 0:
            return zeroval
    ...
    if zeroval:
        zero_pos = where(output_arr == 0)[-1][-1]
        output_arr = output_arr.astype(float)
        output_arr[zero_pos] = zeroval
    return output_arr
...
def ramp(xvals):
    ...
    '''
    unfinished = stepfunction(xvals)
    finished = xvals * unfinished
    return finished


def dirac(t):
    position = where(t == 0)[-1][-1]
    newarr = zeros(len(t))
    newarr[position] = 1
    return newarr
...
def multistem(data, labels):
    ...
    dmin = 0
    for i in range(1, len(data)):
        plt.stem(data[0], data[i], linefmt=colors[i-1],
                 use_line_collection=True, markerfmt=".")
...
def fourier(expression, var, useT=False):
    '''
    ahhhhhhhhhh, if we don't substitute, we can use T and get T directly insteead of w,
    because T = 2pi/w
    '''
    ...
    if not useT:
        transform = fourier_transform(expression, var, k)
        transform1 = transform.subs(k, w / (2 * pi_))
    else:
        transform1 = fourier_transform(expression, var, T)
    return transform1


def ifourier(expression, var, useT=False):
    '''
    ahhhhhhhhhh, if we don't substitute, we can use T and get T directly insteead of w,
    because T = 2pi/w
    '''
    ...
    T = symbol("T", complex=True)
    if not useT:
        expression = expression.subs(w, (k * 2 * pi_))
        transform = inverse_fourier_transform(expression, k, var)
    else:
        transform = inverse_fourier_transform(expression, T, var)
    return transform
